...
//// 2. root -l -b -q 'macros/offlineEtaBin_multiFile.C++("outputs/purityHists_June9",4,"ALL",kFALSE)'
...
void offlineEtaBin_multiFile(const char* FileName="test", Int_t trig=4, const char* Cuts="BEMC",Bool_t ishft=kFALSE);
void checkBatchMode();
Bool_t checkMakePDF();
...
{
  offlineEtaBin_multiFile(FileName,trig,Cuts,ishft);
}

void offlineEtaBin_multiFile(const char* FileName, Int_t trig, const char* Cuts,Bool_t ishft) //0=MB,1=HT1,2=HT2,3=HT3,4=ALL, BEMC, SMD, TOF
{
    ...
    for(int centbin=0;centbin<numCentBins;centbin++) {
      for(int etabin=0;etabin<numEtaBins;etabin++) {
        for(int parnum=0; parnum<numparams; parnum++) {
          ...
          else fitTotal[ier][centbin][etabin][parnum] = new TF1("Junk","pol0",0,20);
        }
      }
    }
  }
...
void makeHist(const char* FileName, Int_t trig,const char* Cut)
{
  ...
  double pT[numCentBins][numEtaBins][numPtBins],
         ...
         purityCount[numCentBins][numEtaBins][numPtBins],
         XnDOF[numCentBins][numEtaBins][numPtBins];
  double pTErr[numCentBins][numEtaBins][numPtBins],
  ...
  ifstream fin(Form("%s.list",FileName));
  TString fname;
  if(fin.is_open())
  {
    while(fin >> fname)
    {
      ...
      else if (f->IsOpen())
      {
        ...
        for(int centbin = 0; centbin<numCentBins; centbin++)
        {
          ...
          nSigmaEPtEta = nSigmaESparse->Projection(0,1,5);
          ...
          for(Int_t etabin=0; etabin<numEtaBins; etabin++)
          {
            ...
            {
              projnSigmaEtemp[centbin][etabin][ptbin]  = (TH1D*)nSigmaEPtEta->ProjectionY(Form("projnSigmaEtemp_%i_%i_%i",centbin,etabin,ptbin),nSigmaEPtEta->GetXaxis()->FindBin(lowpt[ptbin]+0.001),nSigmaEPtEta->GetXaxis()->FindBin(highpt[ptbin]-0.001),nSigmaEPtEta->GetZaxis()->FindBin(loweta[etabin]+0.001),nSigmaEPtEta->GetZaxis()->FindBin(higheta[etabin]-0.001));
              if(DEBUG) cout << "Projection Worked " << centbin << " " << etabin << " " << ptbin << endl;
              if(counter == 0)
                projnSigmaE[centbin][etabin][ptbin] = (TH1D*)projnSigmaEtemp[centbin][etabin][ptbin]->Clone();
              else
                projnSigmaE[centbin][etabin][ptbin]->Add(projnSigmaEtemp[centbin][etabin][ptbin]);
            }
          }
        }
      }
    ...
    cout << "All Files Read In and Projected!" << endl;
  }
  char oname[100];
  TFile* file;
  if(DEBUG) cout << "Make Root: " << makeROOT << endl;
  if(makeROOT){
    sprintf(oname,"%s_Eta_%s_%s_processed.root",FileName,FileLabel,Cuts);
    if(wHFT)
      sprintf(oname,"%s_Eta_%s_%s_wHFT_processed.root",FileName,FileLabel,Cuts);
    file = new TFile(oname,"RECREATE");
...
/*        // Apply parameter constraints
        ...
        for(int q=0; q<15; q++)
        {
          fitCom[centbin][etabin][ptbin]->SetParLimits(q,anaConst::lowLimit[q],anaConst::highLimit[q]);
         if(wFitConstraint&&(q==1||q==4||q==7)) {
            float parFitVal = fitTotal[1][centbin][etabin][q]->Eval(0.5*(lowpt[ptbin]+highpt[ptbin]));
            cout << 0.5*(anaConst::lpt[ptbin]+anaConst::hpt[ptbin]) << " " << 0.92*parFitVal << " " << 1.08*parFitVal << endl;
            fitCom[centbin][etabin][ptbin]->SetParLimits(q,0.92*parFitVal,1.08*parFitVal);
          }
          if(wFitConstraint&&(q==2||q==5||q==8)) {
            float parFitVal = fitTotal[1][centbin][etabin][q]->Eval(0.5*(lowpt[ptbin]+highpt[ptbin]));
            cout << 0.5*(lowpt[ptbin]+highpt[ptbin]) << " " << 0.92*parFitVal << " " << 1.08*parFitVal << endl;
            fitCom[centbin][etabin][ptbin]->SetParLimits(q,0.92*parFitVal,1.08*parFitVal);
          }
        }
