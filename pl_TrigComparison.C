...
int pl_TrigComparison(const char* baseName = "outputs/purityHists_pA15"){
  ...
  //const char* baseName = "outputs/wConstraint/purityHists_pp_July29";
  ...
  for(int centbin = 0; centbin<numCentBins; centbin++)
  {
    for(int etabin=0; etabin<numEtaBins; etabin++)
    {
      ...
      sprintf(textLabel,"Run 15 p+A 200 GeV");
  ...
  char name[100];
  sprintf(name, "%s_purityOverlays_TrigCompare.pdf[", baseName);
  temp->Print(name);
  sprintf(name, "%s_purityOverlays_TrigCompare.pdf", baseName);
